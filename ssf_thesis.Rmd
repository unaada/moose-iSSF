...
# Local roads
local_season_preds <- calculate_season_ci(model_glmm2.0.2, steps3, "local_d")
...
# Prediction function for state roads across all time periods
create_time_predictions <- function(model_object, state_d_center, state_d_scale, max_dist = 2500) {
  # Get coefficients directly from the model object (based on your output)
  beta_state_d <- model_object$fit$par[3]  # State_d base effect (day)
  beta_night_interaction <- model_object$fit$par[10]  # Night interaction 
  beta_twilight_interaction <- model_object$fit$par[11]  # Twilight interaction
  
  # Create distance sequence in original units
  distances <- seq(0, max_dist, length.out = 100)
  
  # Scale the distances to match your model's scaling
  scaled_distances <- (distances - state_d_center) / state_d_scale
  
  # Calculate effects for each time period
  day_effect <- beta_state_d
  night_effect <- beta_state_d + beta_night_interaction
  twilight_effect <- beta_state_d + beta_twilight_interaction
  ...
  for (time_cat in c("Day", "Night", "Twilight")) {
    ...
    if (time_cat == "Day") {
      effect <- day_effect
    } else if (time_cat == "Night") {
      effect <- night_effect
    } else { # Twilight
      effect <- twilight_effect
    }
    
    # Calculate linear predictor (log scale)
    linear_pred <- scaled_distances * effect
    
    # Convert to response scale (exp for Poisson)
    response <- exp(linear_pred)
    
    # Normalize within each time period
    relative_prob <- response / max(response)
    
    # Create data frame for this time period
    ...
    all_predictions <- rbind(all_predictions, time_data)
  }
  
  # Convert time_category to factor with desired order
  all_predictions$time_category <- factor(all_predictions$time_category, 
                                        levels = c("Day", "Night", "Twilight"))
  
  # Create plot
  p <- ggplot(all_predictions, aes(x = distance, y = probability, 
                                  color = time_category, linetype = time_category)) +
    geom_line(size = 1.2) +
    scale_color_manual(values = c("Day" = "#FFC107", "Night" = "#3F51B5", "Twilight" = "#9C27B0")) +
    scale_linetype_manual(values = c("Day" = "solid", "Night" = "dashed", "Twilight" = "dotted")) +
    labs(
      title = "Moose Selection Probability vs. Distance from State Roads",
      x = "Distance (m)",
  ...
  return(list(data = all_predictions, plot = p))
}

# Example usage:
# These are the scaling parameters from your model
state_d_center <- 1895
state_d_scale <- 1175

# Generate the prediction and plot
result <- create_time_predictions(model_glmm3.0.1, state_d_center, state_d_scale)

result$plot

# Display the plot (in R, you would just use: print(result$plot))
