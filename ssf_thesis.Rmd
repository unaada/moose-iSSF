...
```{r all moose}
all_moose <- dat2$dat.resample %>%
  bind_rows() %>%  # This combines all moose into one dataset
  filter_min_n_burst(min_n = 3)
...
steps3$time_category <- factor(steps3$time_category,
                              levels = 1:3,
...
model_base2 <- fit_issf(
  ...
  formula = case_ ~ landuse_factor + factor(new_forest) + 
    ...
    log(sl_) + I(log(sl_)^2) + cos(ta_) +
    log(sl_):landuse_factor + cos(ta_):landuse_factor +
    cos(ta_):minor_d + cos(ta_):major_d + cos(ta_):local_d +
    log(sl_):minor_d + log(sl_):major_d + log(sl_):local_d +
    strata(step_id_)
)
