...
# Claude-e code for doing backwards parameter dropping one by one in a loop
...
#distance to nearest road for each step
...
#mean step length by distance band
...
calculate_crossing_ratio <- function(data, road_type = "state") {
  ...
  # observed crossings (actual moose steps)
  ...
  #  expected crossings (random steps)
  ...
  #  ratio (accounting for 10:1 sampling of random:observed steps)
...
calculate_basic_ci <- function(model, data, road_var, max_dist = 2000) {
  # scaling parameters - calculate directly
...
calculate_time_ci <- function(model, data, road_var, max_dist = 2000) {
  ...
  for (time_cat in time_cats) {
    if (time_cat == "Day") {
      ...
      #interaction term uses time_category (not factor(time_category))
    ...
    # linear predictor, selection strength, and probability
    linear_pred <- scaled_seq * effect
    linear_ci_lower <- linear_pred - 1.96 * effect_se * abs(scaled_seq)
    linear_ci_upper <- linear_pred + 1.96 * effect_se * abs(scaled_seq)
