...
# State roads
state_season_preds <- calculate_season_predictions(model_glmm2.0.2, steps3, "state_d")
state_season_plot <- plot_season_predictions(state_season_preds, "state", "probability")
state_season_plot

# Local roads
local_season_preds <- calculate_season_predictions(model_glmm2.0.2, steps3, "local_d")
...
# Function to create prediction curves specifically for model_glmm3.0.1
create_prediction_curves <- function(model, data, variable = "state_d", 
                                    landuse = "Coniferous", log_sl_val = 0) {
  
  # First check if we're using the correct model type
  if (!inherits(model, "glmmTMB")) {
    # If we have a model that was created with doFit = FALSE
    # We need to fit it first
    if (is.list(model) && "fit" %in% names(model)) {
      # This is a partially fitted glmmTMB model
      # We need to use the parameters that were already estimated
      
      # Create a new prediction function
      predict_func <- function(newdata) {
        # Get fixed effects coefficients
        beta <- model$fit$par[1:length(model$parameters$beta)]
        
        # Create the linear predictor manually
        X <- model_matrix(model, newdata)
        
        # Calculate linear predictor
        eta <- as.numeric(X %*% beta)
        
        # Apply link function (log for Poisson)
        return(eta)
      }
    } else {
      stop("Model must be a fitted glmmTMB object or a list with fit components")
    }
  } else {
    # If we have a proper glmmTMB object, we can use predict directly
    predict_func <- function(newdata) {
      predict(model, newdata = newdata, re.form = NA, type = "link")
    }
  }
  
  # Get range of the variable to predict along
  var_range <- range(data[[variable]], na.rm = TRUE)
  
  # Create sequence of values for x-axis
  pred_values <- seq(var_range[1], var_range[2], length.out = 100)
  
  # Get scaled parameters to back-transform to original scale
  if(variable == "state_d") {
    center <- attr(data$state_d, "scaled:center")
    scale <- attr(data$state_d, "scaled:scale")
    orig_values <- pred_values * scale + center
  } else if(variable == "local_d") {
    center <- attr(data$local_d, "scaled:center")
    scale <- attr(data$local_d, "scaled:scale")
    orig_values <- pred_values * scale + center
  } else {
    orig_values <- pred_values
  }
  
  # Create prediction data frames for each time category
  time_categories <- c("Day", "Night", "Twilight")
  
  # Store results
  all_predictions <- data.frame()
  
  # Create predictions for each time category
  for(time_cat in time_categories) {
    ...
    if(variable == "state_d") {
      newdata <- data.frame(
        log_sl_ = log_sl_val,
        state_d = pred_values,
        local_d = mean(data$local_d, na.rm = TRUE),
        landuse_factor = factor(landuse, levels = levels(data$landuse_factor)),
        "factor(time_category)" = factor(time_cat, levels = levels(data$`factor(time_category)`))
      )
    } else if(variable == "local_d") {
      newdata <- data.frame(
        log_sl_ = log_sl_val,
        state_d = mean(data$state_d, na.rm = TRUE),
        local_d = pred_values,
        landuse_factor = factor(landuse, levels = levels(data$landuse_factor)),
        "factor(time_category)" = factor(time_cat, levels = levels(data$`factor(time_category)`))
      )
    ...
    # Make predictions on the link scale (log scale for Poisson)
    preds <- predict_func(newdata)
    
    # Convert from log to response scale
    probs <- exp(preds)
    
    # Normalize to probability scale (divide by max value)
    max_prob <- max(probs)
    probs_norm <- probs / max_prob
    
    # Combine results
    result <- data.frame(
      distance = orig_values,
...
model_matrix <- function(model, newdata) {
  # Extract terms and contrasts from the model
  terms <- terms(model$fr)
  contrasts <- model$contrasts
  
  # Create the model matrix
  X <- model.matrix(delete.response(terms), newdata, contrasts.arg = contrasts)
  
  # Make sure we have all necessary columns
  # If columns are missing, add them with zeros
  required_cols <- colnames(model$condList$X)
  missing_cols <- setdiff(required_cols, colnames(X))
  
  if (length(missing_cols) > 0) {
    X_add <- matrix(0, nrow = nrow(X), ncol = length(missing_cols))
    colnames(X_add) <- missing_cols
    X <- cbind(X, X_add)
...
# Example usage:
# state_preds <- create_prediction_curves(model_glmm3.0.1, steps3, variable = "state_d", landuse = "Coniferous")
# local_preds <- create_prediction_curves(model_glmm3.0.1, steps3, variable = "local_d", landuse = "Coniferous")

# Function to create plots
plot_predictions <- function(all_preds) {
  ggplot(all_preds, aes(x = distance, y = probability, color = time_category, linetype = time_category)) +
    geom_line(size = 1) +
    facet_wrap(~road_type, scales = "free_x") +
    ...
    labs(
      title = "Predicted Probability of Wolf Presence by Distance to Roads",
      x = "Distance (m)",
...
# Full example of creating both prediction sets and plotting them
create_and_plot_predictions <- function(model, data) {
  # Create prediction curves for state_d (state highway distance)
  state_preds <- create_prediction_curves(model, data, 
                                         variable = "state_d", 
                                         landuse = "Coniferous")
  
  # Create prediction curves for local_d (local road distance)
  local_preds <- create_prediction_curves(model, data, 
                                         variable = "local_d", 
                                         landuse = "Coniferous")
  
  # Combine predictions
  all_preds <- rbind(
    state_preds %>% mutate(road_type = "State Highway"),
    local_preds %>% mutate(road_type = "Local Road")
  )
...
# To create separate plots for different landuse types
create_landuse_plots <- function(model, data) {
  landuse_types <- levels(data$landuse_factor)
  plot_list <- list()
  ...
  for(landuse in landuse_types) {
    ...
    # Plot
    p <- ggplot(state_preds, aes(x = distance, y = probability, color = time_category, linetype = time_category)) +
      geom_line(size = 1) +
      scale_color_manual(values = c("Day" = "#FFC107", "Night" = "#3F51B5", "Twilight" = "#9C27B0")) +
      scale_linetype_manual(values = c("Day" = "solid", "Night" = "dashed", "Twilight" = "dotted")) +
      labs(
        title = paste("Predicted Probability by Distance to State Highway -", landuse),
        x = "Distance (m)",
        y = "Relative Probability",
        color = "Time of Day",
        linetype = "Time of Day"
...
# Example of creating curves with your model:
# 
# # Create and plot predictions
# plot_obj <- create_and_plot_predictions(model_glmm3.0.1, steps3)

# Warning: Unknown or uninitialised column: `factor(time_category)`.Error in EvalADFunObject(ADFun, theta, order = order, hessiancols = cols,  : 
#   Wrong parameter length.
# 
# # Display the plot
# print(plot_obj)
# 
# # Create landuse-specific plots
# landuse_plots <- create_landuse_plots(model_glmm3.0.1, steps3)
# 
# # View individual plots
# print(landuse_plots[["Coniferous"]])
