...
minor_roads_dist <- roads[roads$kal > 500 & roads$kal < 4500,] %>%
  rasterize(landuse) %>%
  distance() %>% 
  crop(study_area) %>% 
  mask(study_area) %>% 
...
local_roads_dist <- roads[roads$kal <= 500 | roads$class == 0,] %>%
  rasterize(landuse) %>%
  distance() %>% 
  crop(study_area) %>% 
  mask(study_area) %>% 
...
ggsave("distance_prop_ratio.png", height = 6, width = 8,  bg = "white", device = png)
